...
public struct PublicKey {
    ...
    public init?(bytes: [UInt8]?) {
        guard let solanaKey = Solana.PublicKey(bytes: bytes) else {
            return nil
        }
    ...
    public init?(string: String) {
        guard let solanaKey = Solana.PublicKey(string: string) else {
            return nil
        }
        self.init(solanaKey: solanaKey)
    }
    ...
    public init?(data: Data) {
        guard let solanaKey = Solana.PublicKey(data: data) else {
            return nil
        }
        self.init(solanaKey: solanaKey)
    }

    internal init(solanaKey: Solana.PublicKey) {
