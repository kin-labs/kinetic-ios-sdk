...
/**
 * A memo format understood by the KRE.
...
public struct KineticKinMemo {
    ...
    public init(magicByteIndicator: UInt8 = 0x1,
        ...
        var foreignKeyBytesPadded = [UInt8].init(repeating: 0, count: Constants.byteLengthForeignKey)
    ...
    public init?(data: Data) throws {
        ...
        var foreignKey = [UInt8](repeating: 0, count: 29)
        for i in 0...27 {
    ...
    public func encode() -> Data {
        ...
        var result = [UInt8](repeating: 0, count: Constants.totalByteCount)

        result[0] = UInt8(magicByteIndicator)
        result[0] |= UInt8(version) << 2
        result[0] |= (UInt8(typeId.rawValue) & 0x7) << 5

        result[1] = (UInt8(typeId.rawValue) & 0x1c) >> 2
        result[1] |= UInt8(appIdx & 0x3f) << 2

        result[2] = UInt8((appIdx & 0x3fc0) >> 6)

        result[3] = UInt8((appIdx & 0xc000) >> 14)
